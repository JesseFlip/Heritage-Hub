...
export default function EventDetail({ slug }: { slug: string }) {
  ...
  // Callers key this component by `slug` so a slug change remounts it with
  // fresh state instead of needing to reset state manually in this effect.
  useEffect(() => {
    getEventBySlug(slug).then((e) => (e ? setEvent(e) : setNotFound(true)));
    try {
      const s: string[] = JSON.parse(localStorage.getItem("hh-saved") || "[]");
      // eslint-disable-next-line react-hooks/set-state-in-effect -- localStorage is client-only; must read after mount, not during the static-export build render.
      setSaved(s.includes(slug));
    } catch {}
  }, [slug]);
